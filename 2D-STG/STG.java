...
class AimBullet extends Bullet
{
   ...
   private double tx,ty,dx,dy;
   private double distance;
   Color color = Color.red;
   AimBullet(double mx,double my,double tx,double ty,double power,double speed)
   {
      this.x = mx;
      this.y = my;
      this.tx = tx;
      ...
      if(this.distance <= 300.0){this.distance = 300.0;}
      else if(this.distance >= 400.0){this.distance = 400.0;}
      this.angle = Math.atan2(dy,dx);
      this.power = power;
      this.speed = speed;
   }

   AimBullet(double mx,double my,double tx,double ty,double power,double speed,Color color)
   {
      this.x = mx;
      this.y = my;
      this.tx = tx;
   ...
   public void Update()
   {
      this.x += Math.cos(angle) * (this.distance / speed);
      this.y += Math.sin(angle) * (this.distance / speed);
   }
...
class GeneralEnemyPlane extends Plane
{
   ...
   private long nDelay = 0;
   private final long aTime = 1000000000;
   private final long nTime = 2000000000;
   ...
   public int Fire(List Bullet,double tx,double ty)
   {
      int num = 3;
      double angle = 360 / num;
      double speed = 100;
      double atk = 20;
      if((System.nanoTime() - aDelay) > aTime){
	 aDelay = System.nanoTime();
	 for(int i = 0;i < num;i++){
	    Bullet.add(new AimBullet(this.x,this.y,tx,ty,atk,speed + (i * 15),Color.pink));
	 }
	 return num;
      }
      return 0;
   ...
   public int Fire2(List Bullet)
   {
      int num = (int)(Math.random() * 12 + 12);
      double angle = 360 / num;
      double speed = 2.8;
      double atk = 8;
      if((System.nanoTime() - nDelay) > nTime){
	 nDelay = System.nanoTime();
	 for(int i = 0;i < num;i++){
	...
	implements KeyListener, Runnable
{
   ...
   long lTime = 0;
   List<GeneralEnemyPlane> enemy = new LinkedList<GeneralEnemyPlane>();
   List<Bullet> aBullet = new LinkedList<Bullet>();
   List<Bullet> nBullet = new LinkedList<Bullet>();

   ListIterator<Bullet> BulletIter;
   ListIterator<GeneralEnemyPlane> PlaneIter; 
   ...
   public void paint(Graphics g)
   {
	 ...
	 b.Draw(g);
      ...
      BulletIter = aBullet.listIterator();
      while(BulletIter.hasNext()){
	 Bullet b = BulletIter.next();
	 b.Draw(g);
      }
   ...
   public void updateObjects()
   {
	 ...
	 if(b.Remove()){
      ...
      while(BulletIter.hasNext()){
	 Bullet b = BulletIter.next();
	 ListIterator<GeneralEnemyPlane> ene = enemy.listIterator();
	 while(ene.hasNext()){
	    Plane f = ene.next();
	 ...
	 if(b.Remove() || b.hp < 0){PlaneIter.remove();}
      ...
      PlaneIter = enemy.listIterator();
      while(PlaneIter.hasNext()){
	 GeneralEnemyPlane p = PlaneIter.next();
	 totalBullet += p.Fire(aBullet,my.GetX(),my.GetY());
	 totalBullet += p.Fire2(nBullet);
      }
